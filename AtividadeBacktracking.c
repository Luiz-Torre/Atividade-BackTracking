...
void imprimirCombinacoes_tarefa3(int valor,int pos, int* vet,int* vetAux,int* vetorFull,int valorOrigin,int soma);
...
void imprimirCombinacoes_tarefa3(int valor,int pos,int* vet,int* vetAux,int* vetorFull,int valorOrigin,int soma){
    ...
    for (int j = 0; j < 5; j++) {
        if (valor - vet[j] >= 0){
            vetorFull[j] += 1;
            imprimirCombinacoes_tarefa3((valor - vet[j]),pos+1,vet,vetAux,vetorFull,valorOrigin,soma);
        }
    }
